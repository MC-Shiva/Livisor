...
// DTO の TransportState は Domain の Transport（再生状態）と 演出キューを 1 つに
...
public static class TransportMapper
{
    public static TransportState ToDto(Room room, long serverTimeMs) => new()
    {
        ...
        Actions = room.DefaultActions.Concat(room.ScheduledActions)
            .OrderBy(action => action.Offset.TotalCentiseconds)
            .Select(ScheduledActionMapper.ToDto).ToArray(),
    };
}
